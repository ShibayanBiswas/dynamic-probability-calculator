...
> **Doc refresh:** 2026-07-16 — Product Specs 27 fields; Coupon / PR / DM internal parse only.

Source: **`New Product Master_.xlsx`** → desk book from **NEW PRIMARY** (Primary + Rollover merged at parse, or baked tab)  
Columns: **Product Explanation**, **Formulae**, **Trade Amount**, **Listing**, **Principal Protection**, etc.

**Not displayed on desk:** raw **Coupon / PR / DM** / **Coupon Participation Return** prose — kept in master for internal coupon parsing (`getCouponPercent()` prefers `CC1:` / first `%` token). Product Specs show **Coupon Percentage** only (27 fields).
